		...
		try {
			...
			if (count($permissions) > 0) {
				...
				foreach ($permissions as $pid) {
					$this->db->query("INSERT INTO acl (permission_id, group_id, created) VALUES (?, ?, ?)",
						$lastInsertID, $pid, TIMESTAMP);
				}
			}
	...
	public function update($id, $nav_path, $permissions = [])
	{
		try {
			...
			if (count($permissions) > 0) {
				...
				foreach ($permissions as $pid) {
					$this->db->query("INSERT INTO acl (permission_id, group_id, created) VALUES (?, ?, ?)", $id, $pid,
						TIMESTAMP);
				}
			} else {
				$this->db->query("DELETE FROM acl WHERE permission_id = ?", $id);
